...
async function rerank(
  ...
  _model: string = DEFAULT_RERANK_MODEL,
  _db?: Database,
...
function showStatus(): void {
  ...
  const collections = listCollectionsFromDB(db);
...
function contextCheck(): void {
  ...
  const allCollections = listCollectionsFromDB(db);
  ...
  if (collectionsWithoutContext.length > 0) {
    ...
    for (const coll of collectionsWithoutContext) {
      console.log(
        `${c.cyan}${coll.name}${c.reset} ${c.dim}(${coll.doc_count} documents)${c.reset}`,
      );
      console.log(
        `  ${c.dim}Suggestion: qmd context add qmd://${coll.name}/ "Description of ${coll.name}"${c.reset}\n`,
      );
    }
  }

  // Check for top-level paths without context within collections that DO have context
  const collectionsWithContext = allCollections.filter(
    (c: { name: string }) =>
  ...
  for (const coll of collectionsWithContext) {
    ...
    if (missingPaths.length > 0) {
      if (!hasPathSuggestions) {
        console.log(
          `${c.yellow}Top-level directories without context:${c.reset}\n`,
        );
      ...
      for (const path of missingPaths) {
        console.log(`  ${path}`);
        console.log(
          `    ${c.dim}Suggestion: qmd context add qmd://${coll.name}/${path} "Description of ${path}"${c.reset}`,
        );
      }
      console.log("");
    }
  ...
  if (collectionsWithoutContext.length === 0 && !hasPathSuggestions) {
    console.log(
      `${c.dim}All collections and major paths have context configured.${c.reset}`,
    );
    console.log(
      `${c.dim}Use 'qmd context list' to see all configured contexts.${c.reset}\n`,
    );
  }
...
function collectionList(): void {
  ...
  for (const coll of collections) {
    ...
    console.log(
      `${c.cyan}${coll.name}${c.reset} ${c.dim}(qmd://${coll.name}/)${c.reset}`,
    );
...
async function vectorIndex(
  ...
  process.stderr.write(
    `Chunking ${hashesToEmbed.length} documents by token count...\n`,
  );
  ...
  if (multiChunkDocs > 0) {
    console.log(
      `${c.dim}${multiChunkDocs} documents split into multiple chunks${c.reset}`,
    );
  }
...
function outputResults(
  ...
  if (opts.format === "json") {
    ...
    const output = filtered.map((row) => {
      ...
      let snippet = !opts.full
        ? extractSnippet(row.body, query, 300, row.chunkPos).snippet
        : undefined;
      if (opts.lineNumbers) {
    ...
    for (const row of filtered) {
      ...
      console.log(
        `#${docid},${row.score.toFixed(2)},${toQmdPath(row.displayPath)}${ctx}`,
      );
    }
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      let displaySnippet = opts.lineNumbers
        ? addLineNumbers(snippet, line)
        : snippet;
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      let content = opts.full
        ? row.body
        : extractSnippet(row.body, query, 500, row.chunkPos).snippet;
      if (opts.lineNumbers) {
    ...
    for (const row of filtered) {
      ...
      console.log(
        `<file docid="#${docid}" name="${toQmdPath(row.displayPath)}"${titleAttr}${contextAttr}>\n${content}\n</file>\n`,
      );
    }
  } else {
    // CSV format
    ...
    for (const row of filtered) {
      ...
      console.log(
        `#${docid},${row.score.toFixed(4)},${escapeCSV(toQmdPath(row.displayPath))},${escapeCSV(row.title || "")},${escapeCSV(row.context || "")},${line},${escapeCSV(snippetText)}`,
      );
    }
  }
}
...
async function vectorSearch(
  ...
  process.stderr.write(
    `${c.dim}Searching ${vectorQueries.length} vector queries...${c.reset}\n`,
  );
...
async function expandQueryStructured(
  ...
  for (let i = 0; i < queryables.length; i++) {
    ...
    const label =
      q.type === "lex" ? "lexical" : q.type === "hyde" ? "hyde" : "vector";
...
async function expandQuery(
  query: string,
  _model: string = DEFAULT_QUERY_MODEL,
  _db?: Database,
): Promise<string[]> {
...
async function querySearch(
  ...
  if (hasStrongSignal) {
    ...
    process.stderr.write(
      `${c.dim}Strong BM25 signal (${topScore.toFixed(2)}) - skipping expansion${c.reset}\n`,
    );
  ...
  process.stderr.write(
    `${c.dim}Searching ${ftsQueries.length} lexical + ${vectorQueries.length} vector queries...${c.reset}\n`,
  );
  ...
  const aggregatedScores = new Map<
    string,
    { score: number; bestChunkIdx: number }
  >();
  for (const r of reranked) {
    const chunkInfo = docChunkMap.get(r.file);
    aggregatedScores.set(r.file, {
      score: r.score,
      bestChunkIdx: chunkInfo?.bestIdx ?? 0,
    });
  }
  ...
  const finalResults = Array.from(aggregatedScores.entries())
    .map(([file, { score: rerankScore, bestChunkIdx }]) => {
      ...
      const chunkBody = chunkInfo
        ? chunkInfo.chunks[bestChunkIdx]?.text || chunkInfo.chunks[0]!.text
        : candidate?.body || "";
...
function showHelp(): void {
  ...
  console.log(
    "  --json/--csv/--md/--xml/--files - Output format (same as search)",
  );
  console.log("");
  ...
  switch (cli.command) {
    case "context": {
      ...
      if (!subcommand) {
        ...
        console.error(
          "  qmd context check               - Check for missing contexts",
        );
    ...
    case "get": {
      if (!cli.args[0]) {
        console.error(
          "Usage: qmd get <filepath>[:line] [--from <line>] [-l <lines>] [--line-numbers]",
        );
        process.exit(1);
      }
    ...
    case "multi-get": {
      if (!cli.args[0]) {
        console.error(
          "Usage: qmd multi-get <pattern> [-l <lines>] [--max-bytes <bytes>] [--json|--csv|--md|--xml|--files]",
        );
        console.error(
    ...
    case "mcp": {
      const { startMcpServer } = await import("@qmd/mcp");
